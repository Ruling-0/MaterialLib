...
/// The damage-value remap that brings a world's stored item shapes from one material id list version to the
/// next.
///
/// Built from a transition's moved and removed index sets: a moved index is rewritten to its new value, and a
/// removed index -- one whose material no longer exists -- deletes its stacks, since the material cannot be
/// shown. Every other index is unchanged. The remap is by damage value alone because one index means the same
/// material across every shape.
public final class MaterialMigration {

    /// [#lookup] result: the stored stack should be removed, its material no longer existing.
